import classes from "./ThemPhuPhi.module.css";
...
const ThemPhuPhi = (props) => {
  const dispatchFn = useDispatch();
  //Des từ props
  const { monthYear, teaSelected } = props;
  ...
  const addPlusWageHandler = (event) => {
    ...
    //Dispatch lên redux
    dispatchFn(LgvActions.addPlusWage(dataSubmit));
    clearInput();
  };
  ...
  const renderPlusWageDates =
    ...
    arrPhuPhiFilter.map((cv) => {
      ...
      const delPlusWageDate = (id) => {
        dispatchFn(LgvActions.delPlusWageDate(id));
      };
      return (
    ...
    const renderTotalRow = totalPlusWage > 0 && <div className={classes.plusDate}>
        <div>Tổng tiền phụ phí : </div>
        <div style={{color:'var(--mauPhu1--',fontWeight:'bold'}}>{formatMoney(totalPlusWage)} đ</div>
    </div>
