...
const LuongCaNhan = (props) => {
  ...
  const renderDataRow = arrDataCaNhanFinal.map((cv) => {
    //Callback kích hoạt hệ số scale
    const active45 = (id) => {
      dispatchFn(LgvActions.changeStuScale({ idStu: id, scale: 45 }));
    };
    const active60 = (id) => {
      dispatchFn(LgvActions.changeStuScale({ idStu: id, scale: 60 }));
    };
